...
def train_one_model(cfg_base: Config, model_name: str):
    # —— 复制一份 cfg，设置当前模型名 —— 
    cfg = Config(**asdict(cfg_base))
    ...
    # —— 针对该模型的输入尺寸构建 dataloader —— 
    train_loader, val_loader, image_size = build_dataloaders(cfg, cfg.MODEL_NAME)

    # —— 构建模型 —— 
    global cfg_global
    cfg_global = cfg  # ☆ 关键：给 mambavision 构建用
    model = create_model(cfg.MODEL_NAME, cfg.NUM_CLASSES, device).to(device)
    ...
    print(f"[{cfg.MODEL_NAME}] Training Start")
    for epoch in range(1, cfg.NUM_EPOCHS + 1):
        if cfg.STAGED_TRAIN and epoch == (cfg.STAGE1_EPOCHS + 1) and stage == 1:
            print(f"\n[{cfg.MODEL_NAME}] [Stage Switch] 解冻骨干：从第 {epoch} 个 epoch 开始全模型训练")
            set_backbone_trainable(model, train_backbone=True, freeze_bn=False)
            stage = 2
            optimizer, scheduler = build_optimizer_and_scheduler(cfg, model, stage=2)

        print(f"\n[{cfg.MODEL_NAME}] ===== Epoch {epoch}/{cfg.NUM_EPOCHS} "
        ...
        print(f"[{cfg.MODEL_NAME}] [Train] loss={train_loss:.4f} acc@1={train_acc1:.2f}%")
        print(f"[{cfg.MODEL_NAME}] [Eval ] loss={val_loss:.4f} acc@1={val_acc1:.2f}% acc@5={val_acc5:.2f}%")
        ...
        if val_acc1 > best_top1:
            ...
            print(f"[{cfg.MODEL_NAME}] [Best Model] New best acc@1={best_top1:.2f}% saved!")

    print(f"\n[{cfg.MODEL_NAME}] 训练完成 | 最佳 acc@1={best_top1:.2f}% | 最佳权重: "
...
if __name__ == "__main__":
    base_cfg = Config(
        TRAIN_DIR="Balanced_Train_Filtered",
        VAL_DIR="Balanced_Val_Filtered",
        MODEL_NAME="swin_v2_t",   # 会被每轮覆盖，这里无所谓
        NUM_CLASSES=19,
        BATCH_SIZE=16,
        NUM_EPOCHS=50,
        USE_AMP=False,
        ...
        LR_HEAD=None,
        LR_BACKBONE=None,
        # 如果需要 Mamba 权重，保持路径正确（可留空）
        MAMBA_T_WEIGHTS="utils_model_checkpoint\mambavision_tiny_1k.pth.tar",
    ...
    for m in MODEL_LIST:
        ...
        print("="*80)
        try:
            train_one_model(base_cfg, m)
        except Exception as e:
            # 若某个模型失败，不影响后续模型继续训练
            print(f"[{m}] 训练失败：{repr(e)}")
            if torch.cuda.is_available(): torch.cuda.empty_cache()
            continue
